...
def demo_classification():
    """XGBoost 分类示例 - 使用乳腺癌数据集"""
    print("=" * 60)
    print("XGBoost - 分类示例 (乳腺癌数据集)")
    print("=" * 60)
    ...
    print(f"\n5 折交叉验证精确率：{cv_scores.mean():.4f} (+/- {cv_scores.std():.4f})")
...
def demo_learning_curve():
    ...
    ax.set_title('XGBoost 学习曲线 - 不同树深的影响', fontsize=12, fontweight='bold')
    ax.legend()
    ax.grid(alpha=0.3)
    
    plt.tight_layout()
    # plt.savefig('xgboost_learning_curve.png', dpi=100, bbox_inches='tight')
    # print("✓ 学习曲线已保存为 'xgboost_learning_curve.png'")
    # plt.close()
    plt.show()
...
def demo_hyperparameter_tuning():
    ...
    ax.set_title('XGBoost 超参数调优热力图', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    # plt.savefig('xgboost_hyperparameter_tuning.png', dpi=100, bbox_inches='tight')
    # print("\n✓ 超参数调优热力图已保存为 'xgboost_hyperparameter_tuning.png'")
    # plt.close()
    plt.show()
